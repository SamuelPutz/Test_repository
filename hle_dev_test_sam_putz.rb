...
def hle_dev_test_sam(client)
  ...
  if new_names.count == 0
	  ...
	  new_names.map do |name|
	  	id = "#{name["id"]}"
	  	clean_name = "#{name["candidate_office_name"].gsub(/^((?![\/]).)*$/){|s| s.downcase}.gsub(/Dist\b/, "District").gsub(/ Rep\b/, " Representative").gsub(/, (.+)(?=\/)/, ' (\1)').gsub(/.+\//){|s| s.downcase }.gsub(/(w+)(Twp)/, '\1 \2').gsub(/Twp.?\b/, "Township").gsub(/\btwp\b/, "township").gsub(/\bHwy\b/, "Highway").gsub(/\bhwy\b/, "highway").gsub(/(.+)\/(.+)\/(.+)/, '\3 \1 and \2').gsub(/, (.+)/, ' (\1)').gsub(/(.+)\/(.+)/, '\2 \1').gsub(/\b(\w+)\s\1\b/i, '\1').gsub(/\b(\w+\s\w+)\s\1/i, '\1').gsub(/  /, ' ').gsub(/(\w\w\w+)(\.)/, '\1').gsub(/mayor$/, "mayor's").strip.split("(").map{|s| if s.chars.include?(")"); s.split(" ").map(&:capitalize).join(" "); else; s; end}.join("(").gsub(/\//, "").gsub(/(\bschool) (district ?\d+) (.+)/, '\2 \1 \3').gsub(/Attorneys? Attorneys?/i, "attorney").gsub(/(.+)(district) (\d+) (\d+)/i, "\1\2 \3").gsub(/(.*)(College)(.*)(community) (college)/i, '\1\2\3').gsub(/(\w+) \w+ \1/i, '\1')}"
